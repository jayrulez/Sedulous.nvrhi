...
class CommandList : RefCounter<nvrhi.d3d12.ICommandList>
{
    ...
    public ID3D12CommandList* getD3D12CommandList() const { return m_ActiveCommandList.commandList; }
    ...
    public void clearTextureFloat(ITexture* _t, TextureSubresourceSet subresources, const Color & clearColor)
    {
    ...
    public void clearDepthStencilTexture(ITexture* _t, TextureSubresourceSet subresources, bool clearDepth, float depth, bool clearStencil, uint8 stencil)
    {
    ...
    public void clearTextureUInt(ITexture* _t, TextureSubresourceSet subresources, uint32 clearColor)
    {
        Texture* t = checked_cast<Texture*>(_t);
    ...
    public void copyTexture(ITexture* _dst, const TextureSlice& dstSlice,
    ...
    public void copyTexture(IStagingTexture* _dst, const TextureSlice& dstSlice, ITexture* _src, const TextureSlice& srcSlice)
    {
        Texture* src = checked_cast<Texture*>(_src);
    ...
    public void copyTexture(ITexture* _dst, const TextureSlice& dstSlice, IStagingTexture* _src, const TextureSlice& srcSlice)
    {
    ...
    public void writeTexture(ITexture* _dest, uint32 arraySlice, uint32 mipLevel, const void* data, int rowPitch, int depthPitch)
    {
        ...
        for (uint32 depthSlice = 0; depthSlice < footprint.Footprint.Depth; depthSlice++)
        {
            for (uint32 row = 0; row < numRows; row++)
            {
                ...
                const void* srcAddress = (const char8*)data + rowPitch * row + depthPitch * depthSlice;
    ...
    public void resolveTexture(ITexture* _dest, const TextureSubresourceSet& dstSubresources, ITexture* _src, const TextureSubresourceSet& srcSubresources)
    {
        Texture* dest = checked_cast<Texture*>(_dest);
        Texture* src = checked_cast<Texture*>(_src);
    ...
    public void writeBuffer(IBuffer* _b, const void * data, int dataSize, uint64 destOffsetBytes)
    {
    ...
    public void clearBufferUInt(IBuffer* _b, uint32 clearValue)
    {
        ...
        {
            std::stringstream ss;
            ss << "Cannot clear buffer " << nvrhi.utils.DebugNameToString(b.desc.debugName)
    ...
    public void copyBuffer(IBuffer* _dest, uint64 destOffsetBytes, IBuffer* _src, uint64 srcOffsetBytes, uint64 dataSizeBytes)
    {
    ...
    public void setPushConstants(const void* data, int byteSize)
    {
        const RootSignature* rootsig = null;
    ...
    public void setGraphicsState(const GraphicsState& state)
    {
        ...
        const bool updateRootSignature = !m_CurrentGraphicsStateValid || m_CurrentGraphicsState.pipeline == null ||
        ...
        if (updateVertexBuffers)
        {
            ...
            for (int i = 0; i < state.vertexBuffers.size(); i++)
            {
                ...
                VBVs[binding.slot].SizeInBytes = (UINT)(std::min(buffer.desc.byteSize - binding.offset, (uint64)ULONG_MAX));
                VBVs[binding.slot].BufferLocation = buffer.gpuVA + binding.offset;

                m_Instance.referencedResources.push_back(buffer);
            }

            uint32 numVertexBuffers = uint32(state.vertexBuffers.size());
            if (m_CurrentGraphicsStateValid)
                numVertexBuffers = std::max(numVertexBuffers, uint32(m_CurrentGraphicsState.vertexBuffers.size()));
        ...
        if (updateShadingRate)
        {
            ...
            else if (m_CurrentGraphicsStateValid && m_CurrentGraphicsState.shadingRateState.enabled)
            {
                ...
                m_ActiveCommandList.commandList6.RSSetShadingRate(D3D12_SHADING_RATE_1X1, null);
            }
        }
    ...
    public void draw(const DrawArguments& args)
    {
    ...
    public void drawIndexed(const DrawArguments& args)
    {
        updateGraphicsVolatileBuffers();
    ...
    public void drawIndirect(uint32 offsetBytes)
    {
        Buffer* indirectParams = checked_cast<Buffer*>(m_CurrentGraphicsState.indirectParams);
        Runtime.Assert(indirectParams); // validation layer handles this
    ...
    public void setComputeState(const ComputeState& state)
    {
        ComputePipeline* pso = checked_cast<ComputePipeline*>(state.pipeline);

        const bool updateRootSignature = !m_CurrentComputeStateValid || m_CurrentComputeState.pipeline == null ||
    ...
    public void dispatch(uint32 groupsX, uint32 groupsY = 1, uint32 groupsZ = 1)
    {
        updateComputeVolatileBuffers();

        m_ActiveCommandList.commandList.Dispatch(groupsX, groupsY, groupsZ);
    }
    public void dispatchIndirect(uint32 offsetBytes)
    {
    ...
    public void setMeshletState(const MeshletState& state)
    {
        ...
        const bool updateRootSignature = !m_CurrentMeshletStateValid || m_CurrentMeshletState.pipeline == null ||
    ...
    public void dispatchMesh(uint32 groupsX, uint32 groupsY = 1, uint32 groupsZ = 1)
    {
        updateGraphicsVolatileBuffers();
    ...
    public void setRayTracingState(const nvrhi.rt.State& state)
    {
        ...
        if (rebuildShaderTable)
        {
            ...
            if (!shaderTable.missShaders.empty())
            {
                ...
                drd.MissShaderTable.SizeInBytes = uint32(shaderTable.missShaders.size()) * entrySize;
            ...
            if (!shaderTable.hitGroups.empty())
            {
                ...
                drd.HitGroupTable.SizeInBytes = uint32(shaderTable.hitGroups.size()) * entrySize;
            ...
            if (!shaderTable.callableShaders.empty())
            {
                ...
                drd.CallableShaderTable.SizeInBytes = uint32(shaderTable.callableShaders.size()) * entrySize;
        ...
        const bool updateRootSignature = !m_CurrentRayTracingStateValid || m_CurrentRayTracingState.shaderTable == null ||
    ...
    public void dispatchRays(const nvrhi.rt.DispatchRaysArguments& args)
    {
        updateComputeVolatileBuffers();
    ...
    public void buildBottomLevelAccelStruct(nvrhi.rt.IAccelStruct* _as, const nvrhi.rt.GeometryDesc* pGeometries, int numGeometries, nvrhi.rt.AccelStructBuildFlags buildFlags)
    {
        AccelStruct* as = checked_cast<AccelStruct*>(_as);

        const bool performUpdate = (buildFlags & nvrhi.rt.AccelStructBuildFlags::PerformUpdate) != 0;
        if (performUpdate)
        {
        ...
        for (uint32 i = 0; i < numGeometries; i++)
        {
            ...
            if (geometryDesc.useTransform)
            {
                ...
                memcpy(cpuVA, &geometryDesc.transform, sizeof(nvrhi.rt.AffineTransform));
        ...
        {
            std::stringstream ss;
            ss << "BLAS " << nvrhi.utils.DebugNameToString(as.desc.debugName) << " build requires at least "
        ...
        {
            std::stringstream ss;
            ss << "Couldn't suballocate a scratch buffer for BLAS " << nvrhi.utils.DebugNameToString(as.desc.debugName) << " build. "
    ...
    public void compactBottomLevelAccelStructs()
    {
#ifdef NVRHI_WITH_RTXMU
    ...
    public void buildTopLevelAccelStruct(nvrhi.rt.IAccelStruct* _as, const nvrhi.rt.InstanceDesc* pInstances, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
    {
        AccelStruct* as = checked_cast<AccelStruct*>(_as);
        ...
        {
            const nvrhi.rt.InstanceDesc& instance = pInstances[i];
        ...
        D3D12_GPU_VIRTUAL_ADDRESS gpuVA = 0;
        int uploadSize = sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * as.dxrInstances.size();
    ...
    public void buildTopLevelAccelStructFromBuffer(nvrhi.rt.IAccelStruct* _as, nvrhi.IBuffer* instanceBuffer, uint64 instanceBufferOffset, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
    {
        AccelStruct* as = checked_cast<AccelStruct*>(_as);
    ...
    public void beginTimerQuery(ITimerQuery* _query)
    {
    ...
    public void endTimerQuery(ITimerQuery* _query)
    {
        TimerQuery* query = checked_cast<TimerQuery*>(_query);
    ...
    public void beginMarker(const char8* name)
    {
    ...
    public void setResourceStatesForBindingSet(IBindingSet* _bindingSet)
    {
    ...
    public void setEnableUavBarriersForTexture(ITexture* _texture, bool enableBarriers)
    {
        Texture* texture = checked_cast<Texture*>(_texture);
    ...
    public void setEnableUavBarriersForBuffer(IBuffer* _buffer, bool enableBarriers)
    {
        Buffer* buffer = checked_cast<Buffer*>(_buffer);
    ...
    public void beginTrackingTextureState(ITexture* _texture, TextureSubresourceSet subresources, ResourceStates stateBits)
    {
        Texture* texture = checked_cast<Texture*>(_texture);

        m_StateTracker.beginTrackingTextureState(texture, subresources, stateBits);
    }
    public void beginTrackingBufferState(IBuffer* _buffer, ResourceStates stateBits)
    {
        Buffer* buffer = checked_cast<Buffer*>(_buffer);
    ...
    public void setTextureState(ITexture* _texture, TextureSubresourceSet subresources, ResourceStates stateBits)
    {
        Texture* texture = checked_cast<Texture*>(_texture);

        m_StateTracker.endTrackingTextureState(texture, subresources, stateBits, false);
    }
    public void setBufferState(IBuffer* _buffer, ResourceStates stateBits)
    {
        Buffer* buffer = checked_cast<Buffer*>(_buffer);

        m_StateTracker.endTrackingBufferState(buffer, stateBits, false);
    }
    public void setAccelStructState(nvrhi.rt.IAccelStruct* _as, ResourceStates stateBits)
    {
        AccelStruct* as = checked_cast<AccelStruct*>(_as);
    ...
    public void setPermanentTextureState(ITexture* _texture, ResourceStates stateBits)
    {
        Texture* texture = checked_cast<Texture*>(_texture);
    ...
    public void setPermanentBufferState(IBuffer* _buffer, ResourceStates stateBits)
    {
        Buffer* buffer = checked_cast<Buffer*>(_buffer);
    ...
    public void commitBarriers()
    {
        ...
        const int barrierCount = textureBarriers.size() + bufferBarriers.size();
        if (barrierCount == 0)
            return;
        ...
        Runtime.Assert(!m_D3DBarriers.empty()); // otherwise there's an early-out in the beginning of this function

        m_ActiveCommandList.commandList.ResourceBarrier(uint32(m_D3DBarriers.size()), m_D3DBarriers.data());
        
        m_StateTracker.clearBarriers();
    }

    public ResourceStates getTextureSubresourceState(ITexture* _texture, ArraySlice arraySlice, MipLevel mipLevel)
    {
        Texture* texture = checked_cast<Texture*>(_texture);

        return m_StateTracker.getTextureSubresourceState(texture, arraySlice, mipLevel);
    }
    public ResourceStates getBufferState(IBuffer* _buffer)
    {
        Buffer* buffer = checked_cast<Buffer*>(_buffer);
    ...
    public nvrhi.IDevice* getDevice()
    {
        return m_Device;
    }
    ...
    public bool allocateUploadBuffer(int size, void** pCpuAddress, D3D12_GPU_VIRTUAL_ADDRESS* pGpuAddress)
    {
    ...
    public bool commitDescriptorHeaps()
    {
    ...
    public D3D12_GPU_VIRTUAL_ADDRESS getBufferGpuVA(IBuffer* _buffer)
    {
        if (!_buffer)
            return 0;
    ...
    public void updateGraphicsVolatileBuffers()
    {
    ...
    public void updateComputeVolatileBuffers()
    {
    ...
    public void setComputeBindings(
        const BindingSetVector& bindings, uint32 bindingUpdateMask,
        IBuffer* indirectParams, bool updateIndirectParams,
        const RootSignature* rootSignature)
    {
        if (bindingUpdateMask)
        {
            StaticVector<VolatileConstantBufferBinding, c_MaxVolatileConstantBuffers> newVolatileCBs;

            for (uint32 bindingSetIndex = 0; bindingSetIndex < uint32(bindings.size()); bindingSetIndex++)
            {
                ...
                {
                    Runtime.Assert(layoutAndOffset.first == _bindingSet.getLayout()); // validation layer handles this

                    BindingSet* bindingSet = checked_cast<BindingSet*>(_bindingSet);

                    // Bind the volatile constant buffers
                    for (int volatileCbIndex = 0; volatileCbIndex < bindingSet.rootParametersVolatileCB.size(); volatileCbIndex++)
                    {
                        ...
                        if (parameter.second)
                        {
                            ...
                            if (buffer.desc.isVolatile)
                            {
                                ...
                                {
                                    std::stringstream ss;
                                    ss << "Attempted use of a volatile constant buffer " << nvrhi.utils.DebugNameToString(buffer.desc.debugName)
        ...
        uint32 bindingMask = (1 << uint32(bindings.size())) - 1;
    ...
    private void bindGraphicsPipeline(GraphicsPipeline *pso, bool updateRootSignature) const
    {
    ...
    private void bindMeshletPipeline(MeshletPipeline *pso, bool updateRootSignature) const
    {
    ...
    private void bindFramebuffer(Framebuffer *fb)
    {
        ...
        for (uint32 rtIndex = 0; rtIndex < fb.RTVs.size(); rtIndex++)
        {
        ...
        m_ActiveCommandList.commandList.OMSetRenderTargets(UINT(RTVs.size()), RTVs.data(), false, fb.desc.depthAttachment.valid() ? &DSV : null);
    }
    ...
    private InternalCommandList createInternalCommandList() const
    {
        ...
        m_Context.device.CreateCommandList(0, d3dCommandListType, commandList.allocator, null, IID_PPV_ARGS(&commandList.commandList));
    ...
    private void buildTopLevelAccelStructInternal(AccelStruct* as, D3D12_GPU_VIRTUAL_ADDRESS instanceData, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
    {
        ...
        {
            Runtime.Assert(as.allowUpdate);
            Runtime.Assert(as.dxrInstances.size() == numInstances); // DXR doesn't allow updating to a different instance count
        }
        ...
        {
            std::stringstream ss;
            ss << "TLAS " << nvrhi.utils.DebugNameToString(as.desc.debugName) << " build requires at least "
                << ASPreBuildInfo.ResultDataMaxSizeInBytes << " bytes in the data buffer, while the allocated buffer is only "
                << as.dataBuffer.desc.byteSize << " bytes";
        ...
        {
            std::stringstream ss;
            ss << "Couldn't suballocate a scratch buffer for TLAS " << nvrhi.utils.DebugNameToString(as.desc.debugName) << " build. "
                "The build requires " << scratchSize << " bytes of scratch space.";
