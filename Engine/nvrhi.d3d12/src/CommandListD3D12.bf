...
class CommandListD3D12 : RefCounter<nvrhi.d3d12.ICommandListD3D12>
{
	// Internal interface functions

	public this(nvrhi.d3d12.DeviceD3D12 device, D3D12Context* context, DeviceResources resources, CommandListParameters @params)
	{
		m_Context = context;
	...
	public CommandListInstance executed(QueueD3D12 pQueue)
	{
	...
	{
		var subresources;
		TextureD3D12 t = checked_cast<TextureD3D12, ITexture>(_t);
	...
	{
		TextureD3D12 dst = checked_cast<TextureD3D12, ITexture>(_dst);
		TextureD3D12 src = checked_cast<TextureD3D12, ITexture>(_src);
	...
	{
		TextureD3D12 src = checked_cast<TextureD3D12, ITexture>(_src);
		StagingTextureD3D12 dst = checked_cast<StagingTextureD3D12, IStagingTexture>(_dst);
	...
	{
		StagingTextureD3D12 src = checked_cast<StagingTextureD3D12, IStagingTexture>(_src);
		TextureD3D12 dst = checked_cast<TextureD3D12, ITexture>(_dst);
	...
	{
		TextureD3D12 dest = checked_cast<TextureD3D12, ITexture>(_dest);
	...
	{
		BufferD3D12 buffer = checked_cast<BufferD3D12, IBuffer>(_b);
	...
	{
		BufferD3D12 dest = checked_cast<BufferD3D12, IBuffer>(_dest);
		BufferD3D12 src = checked_cast<BufferD3D12, IBuffer>(_src);
	...
	public override void setPushConstants(void* data, int byteSize)
	{
		...
		{
			GraphicsPipelineD3D12 pso = checked_cast<GraphicsPipelineD3D12, IGraphicsPipeline>(m_CurrentGraphicsState.pipeline);
			rootsig = pso.rootSignature;
			isGraphics = true;
		}
		else if (m_CurrentComputeStateValid && m_CurrentComputeState.pipeline != null)
		{
			ComputePipelineD3D12 pso = checked_cast<ComputePipelineD3D12, IComputePipeline>(m_CurrentComputeState.pipeline);
			rootsig = pso.rootSignature;
			isGraphics = false;
		}
		else if (m_CurrentRayTracingStateValid && m_CurrentRayTracingState.shaderTable != null)
		{
			RayTracingPipelineD3D12 pso = checked_cast<RayTracingPipelineD3D12, IPipeline>(m_CurrentRayTracingState.shaderTable.getPipeline());
			rootsig = pso.globalRootSignature;
			isGraphics = false;
		}
		else if (m_CurrentMeshletStateValid && m_CurrentMeshletState.pipeline != null)
		{
			MeshletPipelineD3D12 pso = checked_cast<MeshletPipelineD3D12, IMeshletPipeline>(m_CurrentMeshletState.pipeline);
			rootsig = pso.rootSignature;
			isGraphics = true;
		}
	...
	{
		var state;
		GraphicsPipelineD3D12 pso = checked_cast<GraphicsPipelineD3D12, IGraphicsPipeline>(state.pipeline);
		FramebufferD3D12 framebuffer = checked_cast<FramebufferD3D12, IFramebuffer>(state.framebuffer);
		...
		readonly bool updateRootSignature = !m_CurrentGraphicsStateValid || m_CurrentGraphicsState.pipeline == null ||
			checked_cast<GraphicsPipelineD3D12, IGraphicsPipeline>(m_CurrentGraphicsState.pipeline).rootSignature != pso.rootSignature;
		...
		if (updateIndexBuffer)
		{
			...
			{
				BufferD3D12 buffer = checked_cast<BufferD3D12, IBuffer>(state.indexBuffer.buffer);
		...
		if (updateVertexBuffers)
		{
			...
			InputLayoutD3D12 inputLayout = checked_cast<InputLayoutD3D12, IInputLayout>(pso.desc.inputLayout?.Get<IInputLayout>());

			for (int i = 0; i < state.vertexBuffers.Count; i++)
			{
				readonly /*ref*/ VertexBufferBinding binding = /*ref*/ state.vertexBuffers[i];

				BufferD3D12 buffer = checked_cast<BufferD3D12, IBuffer>(binding.buffer);
		...
		if (updateShadingRate || updateFramebuffer)
		{
			...
			if (shouldEnableVariableRateShading)
			{
				...
				TextureD3D12 texture = checked_cast<TextureD3D12, ITexture>(framebufferDesc.shadingRateAttachment.texture);
	...
	{
		BufferD3D12 indirectParams = checked_cast<BufferD3D12, IBuffer>(m_CurrentGraphicsState.indirectParams);
	...
	{
		ComputePipelineD3D12 pso = checked_cast<ComputePipelineD3D12, IComputePipeline>(state.pipeline);

		readonly bool updateRootSignature = !m_CurrentComputeStateValid || m_CurrentComputeState.pipeline == null ||
			checked_cast<ComputePipelineD3D12, IComputePipeline>(m_CurrentComputeState.pipeline).rootSignature != pso.rootSignature;
	...
	{
		BufferD3D12 indirectParams = checked_cast<BufferD3D12, IBuffer>(m_CurrentComputeState.indirectParams);
		Runtime.Assert(indirectParams != null); // validation layer handles this
	...
	{
		var state;
		MeshletPipelineD3D12 pso = checked_cast<MeshletPipelineD3D12, IMeshletPipeline>(state.pipeline);
		FramebufferD3D12 framebuffer = checked_cast<FramebufferD3D12, IFramebuffer>(state.framebuffer);
		...
		readonly bool updateRootSignature = !m_CurrentMeshletStateValid || m_CurrentMeshletState.pipeline == null ||
			checked_cast<MeshletPipelineD3D12, IMeshletPipeline>(m_CurrentMeshletState.pipeline).rootSignature != pso.rootSignature;
	...
	{
		ShaderTableD3D12 shaderTable = checked_cast<ShaderTableD3D12, IShaderTable>(state.shaderTable);
		RayTracingPipelineD3D12 pso = shaderTable.pipeline;
		...
		if (rebuildShaderTable)
		{
			...
			delegate void(ShaderTableD3D12.Entry entry)  writeEntry = scope [=entrySize, &cpuVA, &gpuVA, &entryIndex, &] (entry) =>
				{
					Internal.MemCpy(cpuVA, entry.pShaderIdentifier, D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

					if (entry.localBindings != null)
					{
						d3d12.BindingSetD3D12 bindingSet = checked_cast<d3d12.BindingSetD3D12, IBindingSet>(entry.localBindings?.Get<IBindingSet>());
						d3d12.BindingLayoutD3D12 layout = bindingSet.layout;
		...
		readonly bool updateRootSignature = !m_CurrentRayTracingStateValid || m_CurrentRayTracingState.shaderTable == null ||
			checked_cast<ShaderTableD3D12, IShaderTable>(m_CurrentRayTracingState.shaderTable).pipeline.globalRootSignature != pso.globalRootSignature;
	...
	public override void buildTopLevelAccelStruct(nvrhi.rt.IAccelStruct _as, nvrhi.rt.InstanceDesc* pInstances, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
	{
		...
		for (uint32 i = 0; i < numInstances; i++)
		{
			...
			AccelStructD3D12 blas = checked_cast<AccelStructD3D12, IAccelStruct>(instance.bottomLevelAS);
	...
	public override void setResourceStatesForBindingSet(IBindingSet _bindingSet)
	{
		...
		BindingSetD3D12 bindingSet = checked_cast<BindingSetD3D12, IBindingSet>(_bindingSet);
		...
		for (var bindingIndex in bindingSet.bindingsThatNeedTransitions)
		{
			...
			case ResourceType.RayTracingAccelStruct:
				requireBufferState(checked_cast<AccelStructD3D12, IResource>(binding.resourceHandle).dataBuffer, ResourceStates.AccelStructRead);
				break;
	...
	public override void commitBarriers()
	{
		...
		{
			readonly TextureD3D12 texture = (TextureD3D12)(barrier.texture);
		...
		{
			readonly BufferD3D12 buffer = (BufferD3D12)(barrier.buffer);
			...
			for (uint32 bindingSetIndex = 0; bindingSetIndex < uint32(bindings.Count); bindingSetIndex++)
			{
				...
				if (_bindingSet.getDesc() != null)
				{
					...
					for (int volatileCbIndex = 0; volatileCbIndex < bindingSet.rootParametersVolatileCB.Count; volatileCbIndex++)
					{
						...
						{
							BufferD3D12 buffer = checked_cast<BufferD3D12, IBuffer>(parameter.buffer);
				...
				{
					DescriptorTableD3D12 descriptorTable = checked_cast<DescriptorTableD3D12, IBindingSet>(_bindingSet);
	...
	private struct VolatileConstantBufferBinding
	{
		...
		public BufferD3D12 buffer;
		public D3D12_GPU_VIRTUAL_ADDRESS address;
	}

	private nvrhi.d3d12.IDeviceD3D12 m_Device;
	...
	private void bindGraphicsPipeline(GraphicsPipelineD3D12 pso, bool updateRootSignature)
	{
	...
	private void bindMeshletPipeline(MeshletPipelineD3D12 pso, bool updateRootSignature)
	{
	...
	private void bindFramebuffer(FramebufferD3D12 fb)
	{
		if (m_EnableAutomaticBarriers)
		{
	...
	private void buildTopLevelAccelStructInternal(AccelStructD3D12 @as, D3D12_GPU_VIRTUAL_ADDRESS instanceData, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
	{
