...
namespace nvrhi.vulkan
{
	class CommandList :  RefCounter<ICommandList>
	{
		...
		public this(Device device, VulkanContext* context, CommandListParameters parameters)
		{
			m_Device = device;
			m_Context = context;
		...
		{
			Buffer vkbuf = checked_cast<Buffer, IBuffer>(b);
		...
		public override void setGraphicsState(GraphicsState state)
		{
			...
			Framebuffer fb = checked_cast<Framebuffer, IFramebuffer>(state.framebuffer);
			...
			{
				vkCmdBindIndexBuffer(m_CurrentCmdBuf.cmdBuf, checked_cast<Buffer, IBuffer>(state.indexBuffer.buffer).buffer,
			...
			if (!state.vertexBuffers.IsEmpty && arraysAreDifferent(state.vertexBuffers, m_CurrentGraphicsState.vertexBuffers))
			{
				...
				{
					vertexBuffers.PushBack(checked_cast<Buffer, IBuffer>(vb.buffer).buffer);
		...
		public override void setComputeState(ComputeState state)
		{
			...
			{
				Buffer indirectParams = checked_cast<Buffer, IBuffer>(state.indirectParams);
		...
		public override void buildTopLevelAccelStruct(nvrhi.rt.IAccelStruct _as, nvrhi.rt.InstanceDesc* pInstances, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags)
		{
			...
			for (int i = 0; i < numInstances; i++)
			{
				...
				AccelStruct blas = checked_cast<AccelStruct, IAccelStruct>(src.bottomLevelAS);
#if NVRHI_WITH_RTXMU
		...
		{
			AccelStruct @as = checked_cast<AccelStruct, IAccelStruct>(_as);
			Buffer instanceBuffer = checked_cast<Buffer, IBuffer>(_instanceBuffer);
		...
		public override void commitBarriers()
		{
			...
			for (readonly ref TextureBarrier barrier in ref m_StateTracker.getTextureBarriers())
			{
				...
				Texture texture = (Texture)barrier.texture;
			...
			for (readonly ref BufferBarrier barrier in ref m_StateTracker.getBufferBarriers())
			{
				...
				Buffer buffer = (Buffer)barrier.buffer;
		...
		private void bindBindingSets(VkPipelineBindPoint bindPoint, VkPipelineLayout pipelineLayout, BindingSetVector bindings)
		{
			...
			for ( /*readonly ref*/var bindingSetHandle in ref bindings)
			{
				...
				if (desc != null)
				{
					BindingSet bindingSet = checked_cast<BindingSet, IBindingSet>(bindingSetHandle);
					descriptorSets.PushBack(bindingSet.descriptorSet);

					for (Buffer constnatBuffer in bindingSet.volatileConstantBuffers)
					{
				...
				{
					DescriptorTable table = checked_cast<DescriptorTable, IBindingSet>(bindingSetHandle);
		...
		private void writeVolatileBuffer(Buffer buffer, void* data, int dataSize)
		{
		...
		private void updateMeshletVolatileBuffers()
		{
			...
			{
				MeshletPipeline pso = checked_cast<MeshletPipeline, nvrhi.IMeshletPipeline>(m_CurrentMeshletState.pipeline);
		...
		private void updateRayTracingVolatileBuffers()
		{
			...
			{
				RayTracingPipeline pso = checked_cast<RayTracingPipeline, nvrhi.rt.IPipeline>(m_CurrentRayTracingState.shaderTable.getPipeline());
		...
		private void buildTopLevelAccelStructInternal(AccelStruct @as, VkDeviceAddress instanceData, int numInstances, nvrhi.rt.AccelStructBuildFlags buildFlags, uint64 currentVersion)
		{
