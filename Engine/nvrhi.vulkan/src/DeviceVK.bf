...
{
	class DeviceVK : RefCounter<nvrhi.vulkan.IDeviceVK>
	{
		...
		public this(DeviceDesc desc)
		{
			...
			{
				m_Queues[uint32(CommandQueue.Graphics)] = new QueueVK(m_Context, CommandQueue.Graphics, desc.graphicsQueue, (.)desc.graphicsQueueIndex);
			}

			if (desc.computeQueue != .Null)
			{
				m_Queues[uint32(CommandQueue.Compute)] = new QueueVK(m_Context, CommandQueue.Compute, desc.computeQueue, (.)desc.computeQueueIndex);
			}

			if (desc.transferQueue != .Null)
			{
				m_Queues[uint32(CommandQueue.Copy)] = new QueueVK(m_Context, CommandQueue.Copy, desc.transferQueue, (.)desc.transferQueueIndex);
			}
		...
		public QueueVK getQueue(CommandQueue queue) { return m_Queues[int32(queue)]; }
		...
		public override HeapHandle createHeap(HeapDesc d)
		{
			...
			HeapVK heap = new HeapVK(m_Context, m_Allocator);
			heap.desc = d;
			heap.managed = true;
		...
		{
			TextureVK texture = new TextureVK(m_Context, m_Allocator);
		...
		{
			TextureVK texture = checked_cast<TextureVK, ITexture>(_texture);
			HeapVK heap = checked_cast<HeapVK, IHeap>(_heap);
		...
		public override StagingTextureHandle createStagingTexture(TextureDesc desc, CpuAccessMode cpuAccess)
		{
			...
			StagingTextureVK tex = new StagingTextureVK();
			tex.desc = desc;
			...
			tex.buffer = checked_cast<BufferVK, IBuffer>(internalBuffer.Get<IBuffer>());
		...
		public override void* mapStagingTexture(IStagingTexture _tex, TextureSlice slice, CpuAccessMode cpuAccess, int* outRowPitch)
		{
			...
			StagingTextureVK tex = checked_cast<StagingTextureVK, IStagingTexture>(_tex);
		...
		{
			ShaderVK shader = new ShaderVK(m_Context);
		...
		{
			ShaderVK baseShader = checked_cast<ShaderVK, IShader>(_baseShader);
		...
		{
			SamplerVK sampler = new SamplerVK(m_Context);
		...
		{
			InputLayoutVK layout = new InputLayoutVK();
		...
		public override TimerQueryHandle createTimerQuery()
		{
			...
			TimerQueryVK query = new TimerQueryVK(m_TimerQueryAllocator);
		...
		{
			FramebufferVK fb = new FramebufferVK(m_Context);
			fb.desc = desc;
			...
			for (uint32 i = 0; i < desc.colorAttachments.Count; i++)
			{
				...
				TextureVK t = checked_cast<TextureVK, ITexture>(rt.texture);
			...
			if (desc.depthAttachment.valid())
			{
				...
				TextureVK texture = checked_cast<TextureVK, ITexture>(att.texture);
			...
			if (desc.shadingRateAttachment.valid())
			{
				...
				TextureVK vrsTexture = checked_cast<TextureVK, ITexture>(vrsAttachment.texture);
		...
		public override GraphicsPipelineHandle createGraphicsPipeline(GraphicsPipelineDesc desc, IFramebuffer _fb)
		{
			...
			FramebufferVK fb = checked_cast<FramebufferVK, IFramebuffer>(_fb);

			InputLayoutVK inputLayout = checked_cast<InputLayoutVK, IInputLayout>(desc.inputLayout?.Get<IInputLayout>());

			GraphicsPipelineVK pso = new GraphicsPipelineVK(m_Context);
			pso.desc = desc;
			pso.framebufferInfo = fb.framebufferInfo;

			for (readonly ref BindingLayoutHandle _layout in ref desc.bindingLayouts)
			{
				BindingLayoutVK layout = checked_cast<BindingLayoutVK, IBindingLayout>(_layout?.Get<IBindingLayout>());
				pso.pipelineBindingLayouts.PushBack(layout);
			}

			ShaderVK VS = checked_cast<ShaderVK, IShader>(desc.VS?.Get<IShader>());
			ShaderVK HS = checked_cast<ShaderVK, IShader>(desc.HS?.Get<IShader>());
			ShaderVK DS = checked_cast<ShaderVK, IShader>(desc.DS?.Get<IShader>());
			ShaderVK GS = checked_cast<ShaderVK, IShader>(desc.GS?.Get<IShader>());
			ShaderVK PS = checked_cast<ShaderVK, IShader>(desc.PS?.Get<IShader>());
		...
		public override ComputePipelineHandle createComputePipeline(ComputePipelineDesc desc)
		{
			...
			ComputePipelineVK pso = new ComputePipelineVK(m_Context);
			pso.desc = desc;
			...
			ShaderVK CS = checked_cast<ShaderVK, IShader>(desc.CS.Get<IShader>());
		...
		public override MeshletPipelineHandle createMeshletPipeline(MeshletPipelineDesc desc, IFramebuffer _fb)
		{
			...
			MeshletPipelineVK pso = new MeshletPipelineVK(m_Context);
			pso.desc = desc;
			pso.framebufferInfo = fb.framebufferInfo;
			...
			ShaderVK AS = checked_cast<ShaderVK, IShader>(desc.AS.Get<IShader>());
			ShaderVK MS = checked_cast<ShaderVK, IShader>(desc.MS.Get<IShader>());
			ShaderVK PS = checked_cast<ShaderVK, IShader>(desc.PS.Get<IShader>());
		...
		{
			RayTracingPipelineVK pso = new RayTracingPipelineVK(m_Context);
			pso.desc = desc;
			...
			Dictionary<ShaderVK, uint32> shaderStageIndices = scope .(); // shader . index
			...
			for ( /*readonly ref*/var shaderDesc in ref desc.shaders)
			{
				...
				{
					ShaderVK shader = checked_cast<ShaderVK, IShader>(shaderDesc.shader.Get<IShader>());
			...
			for ( /*readonly ref*/var hitGroupDesc in ref desc.hitGroups)
			{
				...
				{
					ShaderVK shader = checked_cast<ShaderVK, IShader>(hitGroupDesc.closestHitShader.Get<IShader>());
					uint32 shaderStageIndex = shaderStageIndices[shader];
					shaderStages[shaderStageIndex] = makeShaderStageCreateInfo(shader, specInfos, specMapEntries, specData);
				...
				{
					ShaderVK shader = checked_cast<ShaderVK, IShader>(hitGroupDesc.anyHitShader.Get<IShader>());
					uint32 shaderStageIndex = shaderStageIndices[shader];
					shaderStages[shaderStageIndex] = makeShaderStageCreateInfo(shader, specInfos, specMapEntries, specData);
				...
				{
					ShaderVK shader = checked_cast<ShaderVK, IShader>(hitGroupDesc.intersectionShader.Get<IShader>());
					uint32 shaderStageIndex = shaderStageIndices[shader];
					shaderStages[shaderStageIndex] = makeShaderStageCreateInfo(shader, specInfos, specMapEntries, specData);
		...
		{
			BindingLayoutVK ret = new BindingLayoutVK(m_Context, desc);
		...
		{
			BindingLayoutVK layout = checked_cast<BindingLayoutVK, IBindingLayout>(_layout);

			BindingSetVK ret = new BindingSetVK(m_Context);
			ret.desc = desc;
			ret.layout = layout;
			...
			for (int bindingIndex = 0; bindingIndex < desc.bindings.Count; bindingIndex++)
			{
				...
				case ResourceType.Texture_SRV:
					{
						readonly var texture = checked_cast<TextureVK, IResource>(binding.resourceHandle);
				...
				case ResourceType.TypedBuffer_UAV:
					{
						readonly var buffer = checked_cast<BufferVK, IResource>(binding.resourceHandle);
				...
				case ResourceType.Sampler:
					{
						readonly var sampler = checked_cast<SamplerVK, IResource>(binding.resourceHandle);
				...
				case ResourceType.RayTracingAccelStruct:
					{
						readonly var @as = checked_cast<AccelStructVK, IResource>(binding.resourceHandle);
		...
		public override DescriptorTableHandle createDescriptorTable(IBindingLayout _layout)
		{
			...
			DescriptorTableVK ret = new DescriptorTableVK(m_Context);
			ret.layout = layout;
		...
		{
			Runtime.Assert(newSize <= checked_cast<DescriptorTableVK, IDescriptorTable>(_descriptorTable).layout.Get<IBindingLayout>().getBindlessDesc().maxCapacity);
		...
		{
			DescriptorTableVK descriptorTable = checked_cast<DescriptorTableVK, IDescriptorTable>(_descriptorTable);
			BindingLayoutVK layout = checked_cast<BindingLayoutVK, IBindingLayout>(descriptorTable.layout.Get<IBindingLayout>());
			...
			for (uint32 bindingLocation = 0; bindingLocation < uint32(layout.bindlessDesc.registerSpaces.Count); bindingLocation++)
			{
				if (layout.bindlessDesc.registerSpaces[bindingLocation].type == binding.type)
				{
					...
					case ResourceType.Texture_SRV:
						{
							/*readonly ref*/ var texture = checked_cast<TextureVK, IResource>(binding.resourceHandle);
					...
					case ResourceType.TypedBuffer_UAV:
						{
							/*readonly ref*/ var buffer = checked_cast<BufferVK, IResource>(binding.resourceHandle);
					...
					case ResourceType.Sampler:
						{
							/*readonly ref*/ var sampler = checked_cast<SamplerVK, IResource>(binding.resourceHandle);
		...
		{
			AccelStructVK @as = new AccelStructVK(m_Context);
			@as.desc = desc;
			...
			if (isManaged)
			{
				...
				BufferVK dataBuffer = checked_cast<BufferVK, IBuffer>(@as.dataBuffer.Get<IBuffer>());
		...
		{
			AccelStructVK @as = checked_cast<AccelStructVK, nvrhi.rt.IAccelStruct>(_as);
		...
		public override CommandListHandle createCommandList(CommandListParameters @params = .())
		{
			...
			CommandListVK cmdList = new CommandListVK(this, m_Context, @params);
		...
		{
			QueueVK queue = m_Queues[uint32(executionQueue)];

			uint64 submissionID = queue.submit(pCommandLists.Ptr, pCommandLists.Length);

			for (int i = 0; i < pCommandLists.Length; i++)
			{
				checked_cast<CommandListVK, ICommandList>(pCommandLists[i]).executed(queue, submissionID);
			}
		...
		{
			QueueVK queue = m_Queues[uint32(queueID)];
		...
		{
			QueueVK waitQueue = m_Queues[uint32(waitQueueID)];
		...
		{
			QueueVK executionQueue = m_Queues[uint32(executionQueueID)];
		...
		private QueueVK[(uint32)CommandQueue.Count] m_Queues = .();
