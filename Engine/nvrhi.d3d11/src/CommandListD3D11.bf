...
class CommandListD3D11 : RefCounter<ICommandList>
{
    public this(D3D11Context* context, IDevice device, CommandListParameters @params){
		m_Context =context;
		m_Device = device;
		m_Desc = @params;
    ...
    {
		var subresources;
        TextureD3D11 texture = checked_cast<TextureD3D11, ITexture>(_texture);
    ...
    public override void clearDepthStencilTexture(ITexture t, TextureSubresourceSet subresources, bool clearDepth, float depth, bool clearStencil, uint8 stencil)
    {
        ...
        TextureD3D11 texture = checked_cast<TextureD3D11, ITexture>(t);
    ...
    {
        TextureD3D11 src = checked_cast<TextureD3D11, ITexture>(_src);
        TextureD3D11 dst = checked_cast<TextureD3D11, ITexture>(_dst);
    ...
    {
        TextureD3D11 src = checked_cast<TextureD3D11, ITexture>(_src);
        StagingTextureD3D11 dst = checked_cast<StagingTextureD3D11, IStagingTexture>(_dst);
    ...
    {
        StagingTextureD3D11 src = checked_cast<StagingTextureD3D11, IStagingTexture>(_src);
        TextureD3D11 dst = checked_cast<TextureD3D11, ITexture>(_dst);
    ...
    {
        TextureD3D11 dest = checked_cast<TextureD3D11, ITexture>(_dest);
    ...
    public override void clearBufferUInt(IBuffer buffer, uint32 clearValue)
    {
        ...
        ID3D11UnorderedAccessView* uav = checked_cast<BufferD3D11, IBuffer>(buffer).getUAV(Format.UNKNOWN, EntireBuffer, viewType);
    ...
    {
        BufferD3D11 dest = checked_cast<BufferD3D11, IBuffer>(_dest);
        BufferD3D11 src = checked_cast<BufferD3D11, IBuffer>(_src);
    ...
    {
        GraphicsPipelineD3D11 pipeline = checked_cast<GraphicsPipelineD3D11, IGraphicsPipeline>(state.pipeline);
        FramebufferD3D11 framebuffer = checked_cast<FramebufferD3D11, IFramebuffer>(state.framebuffer);
        ...
        if (updateFramebuffer || checked_cast<GraphicsPipelineD3D11, IGraphicsPipeline>(m_CurrentGraphicsPipeline?.Get<IGraphicsPipeline>()).pixelShaderHasUAVs != pipeline.pixelShaderHasUAVs)
        {
        ...
        if (updateBindings)
        {
            ...
            if (pipeline.pixelShaderHasUAVs)
            {
                ...
                {
                    BindingSetD3D11 bindingSet = checked_cast<BindingSetD3D11, IBindingSet>(_bindingSet);
        ...
        if (updateVertexBuffers)
        {
            ...
            for (int i = 0; i < state.vertexBuffers.Count; i++)
            {
                ...
                pVertexBuffers[i] = checked_cast<BufferD3D11, IBuffer>(binding.buffer).resource;
        ...
        if (updateIndexBuffer)
        {
            ...
            {
                m_Context.immediateContext.IASetIndexBuffer(checked_cast<BufferD3D11, IBuffer>(state.indexBuffer.buffer).resource,
    ...
    {
        BufferD3D11 indirectParams = checked_cast<BufferD3D11, IBuffer>(m_CurrentIndirectBuffer?.Get<IBuffer>());
    ...
    {
        ComputePipelineD3D11 pso = checked_cast<ComputePipelineD3D11, IComputePipeline>(state.pipeline);
    ...
    public override void endTimerQuery(ITimerQuery _query)
{
    ...
    private void bindGraphicsPipeline(GraphicsPipelineD3D11 pso)
    {
    ...
    ref BindingSetVector outSetsToBind)
{
    ...
    if (currentResourceSets != null)
    {
        ...
        readonly GraphicsPipelineD3D11 currentPipeline = checked_cast<GraphicsPipelineD3D11, IGraphicsPipeline>(_currentPipeline);
        readonly GraphicsPipelineD3D11 newPipeline = checked_cast<GraphicsPipelineD3D11, IGraphicsPipeline>(_newPipeline);
            ...
            if (!updateFramebuffer)
            {
                for (uint32 i = 0; i < uint32(outSetsToBind.Count); i++)
                {
                    if (outSetsToBind[i] != null)
                        for (uint32 j = 0; j < uint32(setsToUnbind.Count); j++)
                        {
                            if (setsToUnbind[j] != null && checked_cast<BindingSetD3D11, IBindingSet>(outSetsToBind[i]).isSupersetOf(checked_cast<BindingSetD3D11, IBindingSet>(setsToUnbind[j])))
                            {
                                setsToUnbind[j] = null;
                            }
                        }
        ...
        for (IBindingSet _set in setsToUnbind)
        {
            ...
            BindingSetD3D11 set = checked_cast<BindingSetD3D11, IBindingSet>(_set);
    ...
    private void bindGraphicsResourceSets(BindingSetVector setsToBind, IGraphicsPipeline newPipeline) {
		for(IBindingSet _set in setsToBind)
		{
		    ...
		    BindingSetD3D11 set = checked_cast<BindingSetD3D11, IBindingSet>(_set);
		    readonly GraphicsPipelineD3D11 pipeline = checked_cast<GraphicsPipelineD3D11, IGraphicsPipeline>(newPipeline);
    ...
    if (currentResourceSets != null)
    {
        ...
        for (uint32 i = 0; i < uint32(setsToBind.Count); i++)
        {
            if (setsToBind[i] != null)
                for (uint32 j = 0; j < uint32(setsToUnbind.Count); j++)
                {
                    BindingSetD3D11 setToBind = checked_cast<BindingSetD3D11, IBindingSet>(setsToBind[j]);
                    BindingSetD3D11 setToUnbind = checked_cast<BindingSetD3D11, IBindingSet>(setsToUnbind[j]);
