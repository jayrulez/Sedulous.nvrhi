...
class Device : RefCounter<nvrhi.IDevice>
{
	public this(DeviceDesc desc)
	{
		...
		m_ImmediateCommandList = CommandListHandle.Attach(new CommandList(m_Context, this, .()));
	}
	...
	public override StagingTextureHandle createStagingTexture(TextureDesc d, CpuAccessMode cpuAccess)
	{
		...
		ret.texture = checked_cast<Texture, ITexture>(t.Get<ITexture>());
	...
	{
		StagingTexture stagingTexture = checked_cast<StagingTexture, IStagingTexture>(_stagingTexture);
	...
	{
		StagingTexture t = checked_cast<StagingTexture, IStagingTexture>(_t);
	...
	private static void createShaderFailed(char8* @function, HRESULT res, ShaderDesc d, Context* context)
	{
	...
	public override ShaderHandle createShader(ShaderDesc d, void* binary, int binarySize)
	{
		...
		RefCountPtr<Shader> shader = RefCountPtr<Shader>.Attach(new Shader());
	...
	{
		Shader vertexShader = checked_cast<Shader, IShader>(_vertexShader);
	...
	// event queries
	private static bool checkedCreateQuery(D3D11_QUERY_DESC queryDesc, char8* name, Context* context, ID3D11Query** pQuery)
	{
		var queryDesc;
	...
	public override GraphicsPipelineHandle createGraphicsPipeline(GraphicsPipelineDesc desc, IFramebuffer fb)
	{
		...
		pso.inputLayout = checked_cast<InputLayout, IInputLayout>(desc.inputLayout.Get<IInputLayout>());
		...
		if (desc.VS != null) { pso.pVS = checked_cast<Shader, IShader>(desc.VS.Get<IShader>()).VS; pso.shaderMask = pso.shaderMask | ShaderType.Vertex; }
		if (desc.HS != null) { pso.pHS = checked_cast<Shader, IShader>(desc.HS.Get<IShader>()).HS; pso.shaderMask = pso.shaderMask | ShaderType.Hull; }
		if (desc.DS != null) { pso.pDS = checked_cast<Shader, IShader>(desc.DS.Get<IShader>()).DS; pso.shaderMask = pso.shaderMask | ShaderType.Domain; }
		if (desc.GS != null) { pso.pGS = checked_cast<Shader, IShader>(desc.GS.Get<IShader>()).GS; pso.shaderMask = pso.shaderMask | ShaderType.Geometry; }
		if (desc.PS != null) { pso.pPS = checked_cast<Shader, IShader>(desc.PS.Get<IShader>()).PS; pso.shaderMask = pso.shaderMask | ShaderType.Pixel; }
	...
	public override ComputePipelineHandle createComputePipeline(ComputePipelineDesc desc)
	{
		...
		if (desc.CS != null) pso.shader = checked_cast<Shader, IShader>(desc.CS.Get<IShader>()).CS;
